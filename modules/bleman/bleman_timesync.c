...
static void _read_handler(event_t *ev)
{
    ...
    puts("[CHR]: Reading characteristic!");
...
void _time_chrs_handler(event_t *ev)
{
    ...
    int res = ble_gattc_disc_chrs_by_uuid(sync->bleman->conn_handle,
                                          ...
                                          _chr_disced, sync);
    if (res != 0) {
        printf("characteristic result : %d\n", res);
    }
}
